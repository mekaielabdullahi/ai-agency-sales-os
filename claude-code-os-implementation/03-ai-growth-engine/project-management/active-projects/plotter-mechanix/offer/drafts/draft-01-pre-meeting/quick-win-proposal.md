...
|---------------|----------|
| Kelsey checks texts going back days to catch missed items | 30+ min/day wasted |
...
| Kelsey calls Alyssa after every job to hand off | Interruptions, no async flexibility |
...
**Cost:** ~$23/user/month billed annually = ~$69/month total for 3 users (Kelsey, Alyssa, tech) or ~$99/month billed monthly
...
| **Service Call Dispatch** | Real-time routing based on Kelsey's location | Your competitive advantage, documented |
...
We'll also set up a simple system where Kelsey can record himself working and narrating—and we turn those recordings into additional SOPs at no extra effort from him:

```
Kelsey narrates while working → Uploads to shared folder → We process into polished SOP
```
...
Every day at 4:00 PM:
  ...
  → Send to Alyssa (and optionally Kelsey)
...
**The problem it solves:**
When Alyssa adds a new contact from a business card Kelsey dropped, he has no visibility until he asks.

**How it works:**
```
When Alyssa adds a new contact to Jobber:
  → Kelsey gets an instant notification
  → Includes: Company, contact name, how they found us, next action
  → Kelsey stays informed without asking
```
...
**The problem it solves:**
Kelsey calls Alyssa after every job. That's 4-6 interruptions per day. By end of day, details are fuzzy.
...
This is the linchpin. Instead of calling Alyssa after every job, Kelsey fills out a structured template in Jobber. Takes 2 minutes. Alyssa gets everything she needs.
...
**Why this works:**
- Kelsey fills it out while details are fresh (at the job site)
...
│  • Morning: Review schedule (Kelsey in hot tub)              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│     → New hire training without Kelsey                       │

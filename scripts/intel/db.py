...
Creates and manages tables for meeting transcripts,
Slack messages, team registry, and collection logs.

Uses the existing DataOS database (data/data.db) so all data
lives in one place.
"""
...
# Use existing DataOS database — everything in one place
...
def init_db() -> sqlite3.Connection:
    """Initialize the database with IntelOS tables."""
    conn = get_connection()
...
def log_collection(conn: sqlite3.Connection, source: str, status: str,
    ...
    """Log a collection run to the existing collection_log table."""
    conn.execute(

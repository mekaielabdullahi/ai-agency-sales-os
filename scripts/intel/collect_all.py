...
Usage:
    ...
    python scripts/intel/collect_all.py --days 30          # Look back 30 days
"""
...
def run(meetings_only: bool = False, slack_only: bool = False, days: int = 7):
    ...
    if not slack_only:
        ...
        try:
            ...
            ff_meetings = collect_fireflies(days=days)
            if ff_meetings:
        ...
        try:
            ...
            fathom_meetings = collect_fathom(days=days)
            if fathom_meetings:
    ...
    if not meetings_only:
        try:
            ...
            if messages:
                ...
                print("Slack: no messages collected (no token configured or no channels)")
        except ImportError:
            print("Slack: collector not installed")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    run(meetings_only=args.meetings_only, slack_only=args.slack_only, days=args.days)
